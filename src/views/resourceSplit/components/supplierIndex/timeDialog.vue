  ...
  <el-dialog title="配置详情" :visible.sync="visible" @open="fetchData" @close='close' width="1200px"
    ...
    <el-table height="460" :data="tableData" :loading="loading">
      ...
      <el-table-column label="申请时长">
        <template slot-scope="{row}">
          <span
            :class='row.applyHours > 0 ? "redStatus" : row.applyHours < 0 ? "greenStatus" : "zero"'>{{
              row.applyHours
              > 0 ? "+" : ""
            }}{{ row.applyHours | diment(row.applyHours) }}</span>
        </template>
      </el-table-column>
...
<script>
import { getSuperDetail } from '@/api/resource';
...
export default {
  ...
  methods: {
    ...
    fetchData(time) {
      ...
      getSuperDetail(params).then(res => {
